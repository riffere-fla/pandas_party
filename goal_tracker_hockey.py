...
@task(log_prints = True)
def create_weekly_table(redshift_credentials: Dict) -> pd.DataFrame:
    ...
    q = """
        ...
        cross_table AS (
            ...
            CROSS JOIN (
                ...
                FROM
                    custom.cte_v_ticket_2425
                WHERE
                    DATE(transaction_date) >= DATE(current_time_est) - 7
            )
        ),
        ticket_info AS (
            SELECT
        ...
        SELECT
            cross_table.sales_rep,
            DATE(cross_table.transaction_date) AS transaction_date,
            event_datetime,
        ...
        LEFT JOIN
            ticket_info
            ON cross_table.sales_rep = ticket_info.sales_rep
            AND DATE(cross_table.transaction_date) = DATE(ticket_info.transaction_date)
        WHERE
            cross_table.sales_rep IS NOT NULL
        GROUP BY
            cross_table.sales_rep,
            DATE(cross_table.transaction_date),
            event_datetime``
    """
...
def goal_tracker_3pe() -> None:
    ...
    create_weekly_table(redshift_credentials)
